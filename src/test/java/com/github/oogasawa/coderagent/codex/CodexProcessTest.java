...
/**
 * Tests for CodexProcess command building (no subprocess execution).
 */
class CodexProcessTest {

    @Test
    void buildCommand_defaultConfig() {
        CodexProcess cp = new CodexProcess(CodexConfig.defaults());
    ...
    void buildCommand_withSystemPrompt() {
        CodexConfig config = CodexConfig.defaults().withSystemPrompt("Be concise");
        CodexProcess cp = new CodexProcess(config);
        List<String> cmd = cp.buildCommand();
    ...
    void buildCommand_withMaxTurns() {
        CodexConfig config = CodexConfig.defaults().withMaxTurns(3);
        CodexProcess cp = new CodexProcess(config);
        List<String> cmd = cp.buildCommand();
    ...
    void buildCommand_withSessionId() {
        CodexConfig config = CodexConfig.defaults().withSessionId("sess-abc-123");
        CodexProcess cp = new CodexProcess(config);
        List<String> cmd = cp.buildCommand();
    ...
    void buildCommand_withAllowedTools() {
        CodexConfig config = CodexConfig.defaults().withAllowedTools("Read", "Bash");
        CodexProcess cp = new CodexProcess(config);
        List<String> cmd = cp.buildCommand();
    ...
    void setConfig_updatesConfig() {
        CodexProcess cp = new CodexProcess(CodexConfig.defaults());
        CodexConfig newConfig = CodexConfig.defaults().withModel("o3");
        cp.setConfig(newConfig);

        assertEquals("o3", cp.getConfig().model());
    }
    ...
    void buildCommand_nullModel_noModelFlag() {
        CodexConfig config = new CodexConfig(null, null, 0, null, null, false, null);
        CodexProcess cp = new CodexProcess(config);
        List<String> cmd = cp.buildCommand();
    ...
    void wrapWithPty_wrapsWithScript() {
        CodexProcess cp = new CodexProcess(CodexConfig.defaults());
        List<String> codexCmd = cp.buildCommand();
        List<String> wrapped = cp.wrapWithPty(codexCmd);
        ...
        assertTrue(wrapped.get(5).contains("codex"));
    ...
    void stripAnsi_removesEscapeSequences() {
        ...
        String clean = CodexProcess.stripAnsi(dirty);
    ...
    void stripAnsi_preservesCleanString() {
        ...
        assertEquals(clean, CodexProcess.stripAnsi(clean));
    }

    @Test
    void stripAnsi_handlesEmptyString() {
        assertEquals("", CodexProcess.stripAnsi(""));
    }

    @Test
    void escapeJsonString_basicString() {
        assertEquals("\"hello\"", CodexProcess.escapeJsonString("hello"));
    }

    @Test
    void escapeJsonString_withQuotes() {
        assertEquals("\"say \\\"hi\\\"\"", CodexProcess.escapeJsonString("say \"hi\""));
    }

    @Test
    void escapeJsonString_withBackslash() {
        assertEquals("\"path\\\\to\\\\file\"", CodexProcess.escapeJsonString("path\\to\\file"));
    }

    @Test
    void escapeJsonString_withNewlines() {
        assertEquals("\"line1\\nline2\"", CodexProcess.escapeJsonString("line1\nline2"));
    }

    @Test
    void escapeJsonString_withTab() {
        assertEquals("\"col1\\tcol2\"", CodexProcess.escapeJsonString("col1\tcol2"));
    }

    @Test
    void escapeJsonString_withControlChar() {
        String input = "before\u0003after";
        String result = CodexProcess.escapeJsonString(input);
