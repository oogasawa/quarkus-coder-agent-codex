...
class StreamEventParserTest {
    ...
    // -------------------------------------------------------------------------
    // system events (actual Codex Code format)
    // -------------------------------------------------------------------------

    @Test
    void parse_systemInit() {
        String json = """
            {"type":"system","subtype":"init","cwd":"/home/user","session_id":"abc-123","model":"codex-sonnet-4-6","tools":["Bash","Read"]}
            """;
        ...
        assertTrue(event.content().contains("codex-sonnet-4-6"));
    ...
    void parse_systemNonInit() {
        String json = """
            {"type":"system","message":"Codex CLI initialized"}
            """;

        StreamEvent event = parser.parse(json);

        assertEquals("system", event.type());
        assertEquals("Codex CLI initialized", event.content());
    }

    // -------------------------------------------------------------------------
    // assistant events (actual Codex Code format)
    // -------------------------------------------------------------------------

    @Test
    void parse_assistantWithText() {
        String json = """
            {"type":"assistant","message":{"model":"codex-sonnet-4-6","content":[{"type":"text","text":"Hello!"}],"role":"assistant"}}
            """;
    ...
    // -------------------------------------------------------------------------
    // result events (actual Codex Code format)
    // -------------------------------------------------------------------------
    ...
    @Test
    void parse_askUserQuestion_realCodexCodeFormat() {
        String json = """
