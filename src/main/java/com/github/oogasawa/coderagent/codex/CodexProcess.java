...
/**
 * Level 1 POJO that manages Codex CLI subprocess execution.
 *
 * <p>Launches {@code codex --print --output-format stream-json "prompt"} as a subprocess
...
public class CodexProcess {
    ...
    private static final Logger logger = Logger.getLogger(CodexProcess.class.getName());
    ...
    /**
     * Constructs a CodexProcess with the specified configuration.
     *
     * @param config the Codex CLI configuration
     */
    public CodexProcess(CodexConfig config) {
        this.config = config;
    }
     ...
     *
     * @return the current CodexConfig
     */
     ...
     */
    public void setConfig(CodexConfig config) {
        this.config = config;
    }

    /**
     * Sets the OpenAI API key for use when Codex CLI is not installed.
     * The key is passed as OPENAI_API_KEY environment variable to the subprocess.
     *
    ...
    /**
     * Returns whether the Codex CLI process is currently running.
     *
    ...
    /**
     * Starts a new Codex CLI subprocess.
     *
     * @throws IOException if the subprocess fails to start
     */
    private void startProcess() throws IOException {
        List<String> codexCmd = buildCommand();

        // With --input-format stream-json, PTY is not needed
        // because Codex CLI accepts piped JSON input directly.
        logger.fine(() -> "Starting Codex CLI: " + String.join(" ", codexCmd));

        ProcessBuilder pb = new ProcessBuilder(codexCmd);

        // Set API key if provided (for users without Codex CLI login)
    ...
    /**
     * Sends a prompt to Codex CLI and streams the response events.
     *
    ...
    /**
     * Cancels the currently running Codex CLI process, if any.
     */
    public void cancel() {
        ...
        if (p != null && p.isAlive()) {
            p.destroy();
            logger.info("Codex CLI process cancelled");
        }
    }

    /**
     * Writes a user message to Codex's stdin in stream-json format.
     *
     ...
     * Codex CLI requires stdout to be a TTY for stream-json output.
     *
     * @param codexCmd the codex command arguments
     * @return command list starting with 'script'
     */
    List<String> wrapWithPty(List<String> codexCmd) {
        // Shell-escape each argument and join into a single command string
        String innerCommand = codexCmd.stream()
            .map(arg -> "'" + arg.replace("'", "'\\''") + "'")
    ...
    /**
     * Builds the Codex CLI command arguments.
     *
    ...
    List<String> buildCommand() {
        ...
        // With --print, Codex CLI runs in non-interactive mode where tool
    ...
    /**
     * Callback interface for receiving stream events from Codex CLI.
     */
