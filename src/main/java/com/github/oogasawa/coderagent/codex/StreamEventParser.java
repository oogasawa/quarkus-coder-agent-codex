...
/**
 * Parses stream-json output lines from Codex CLI into {@link StreamEvent} objects.
 *
 * <p>Codex Code {@code --output-format stream-json} emits these event types:</p>
 * <ul>
 ...
 *   <li>{@code user} — tool results returned by Codex Code</li>
...
public class StreamEventParser {
    ...
    /**
     * Parses a user event (tool result from Codex Code).
     *
     * <p>Format: {@code {"type":"user","message":{"content":[{"type":"tool_result",...}]},
     * "tool_use_result":{...}}}</p>
     *
     * <p>These events indicate that Codex Code has executed a tool and received a result.
    ...
    private StreamEvent parseEmbeddedAskUser(JSONObject toolUseBlock, String rawJson) {
        ...
        if (input == null) {
            return StreamEvent.prompt(id, "Question from Codex", "ask_user",
    ...
    private StreamEvent parseAskUserQuestion(String id, JSONObject input, String rawJson) {
        // Try "questions" array first (actual Codex Code format)
    ...
    private StreamEvent parseSingleQuestion(String id, JSONObject qObj, String rawJson) {
        String question = qObj.optString("question",
            qObj.optString("message", "Question from Codex"));
